...
import { AuthorOnly, PublisherOnly } from '@/components/auth/ProtectedRoute';
import { formatPrice } from '@/lib/utils';
import { getPublisherSalesReport, exportPublisherSalesToCSV } from '@/lib/royalty';
...
const PublisherReportsPage = () => {
  ...
  const [reportData, setReportData] = useState<PublisherReportData[]>([]);
  ...
  const fetchReportData = async () => {
    try {
      ...
      const monthlyData: { [key: string]: PublisherReportData } = {};
      ...
      salesData.forEach((sale: Sale) => {
        ...
        monthlyData[monthKey].publisherEarnings += sale.publisherShare;
      ...
      setTimeout(() => {
        const sampleData: PublisherReportData[] = [
          {
            period: 'Jan 2024',
            totalSales: 1200,
            totalRevenue: 42000,
            authorRoyalties: 12600,
            publisherEarnings: 25200,
            booksSold: 1200
          },
          {
            period: 'Feb 2024',
  ...
  const totalStats = reportData.reduce(
    (acc, data) => ({
      ...
      publisherEarnings: acc.publisherEarnings + data.publisherEarnings,
      booksSold: acc.booksSold + data.booksSold
    }),
    { totalSales: 0, totalRevenue: 0, authorRoyalties: 0, publisherEarnings: 0, booksSold: 0 }
  );
  ...
  const exportReport = async (format: 'csv' | 'excel' | 'pdf') => {
    try {
      ...
      if (format === 'csv') {
        ...
        const salesData = await getPublisherSalesReport(userProfile.userId);
        const csvContent = await exportPublisherSalesToCSV(salesData);
        ...
        link.href = url;
        link.download = `publisher-sales-report-${new Date().toISOString().split('T')[0]}.csv`;
  ...
  return (
    <PublisherOnly>
      <DashboardLayout title="Publisher Reports">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          {/* Header */}
          <div className="flex items-center justify-between mb-8">
            <div>
              <h1 className="text-2xl font-bold text-gray-900 dark:text-white">Publisher Reports</h1>
              <p className="text-gray-600 dark:text-gray-400">Sales and revenue analytics for your publications</p>
            </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
            ...
            <Card>
              <CardContent className="p-6">
                <div className="flex items-center justify-between">
                  <div>
                    <p className="text-sm text-gray-600 dark:text-gray-400">Publisher Earnings</p>
                    <p className="text-2xl font-bold text-gray-900 dark:text-white">
                      {formatPrice(totalStats.publisherEarnings)}
                    </p>
                  </div>
                  ...
                  <table className="w-full">
                    <thead>
                      <tr className="border-b border-gray-200 dark:border-gray-700">
                        ...
                        <th className="text-left py-3 px-4">Publisher Earnings</th>
                    ...
                    <tbody>
                      {reportData.map((data, index) => (
                        <tr key={index} className="border-b border-gray-100 dark:border-gray-800 hover:bg-gray-50 dark:hover:bg-gray-800">
                          ...
                          <td className="py-4 px-4">{formatPrice(data.publisherEarnings)}</td>
                    ...
                    <tfoot>
                      <tr className="bg-gray-50 dark:bg-gray-800 font-semibold">
                        ...
                        <td className="py-4 px-4">{formatPrice(totalStats.publisherEarnings)}</td>
