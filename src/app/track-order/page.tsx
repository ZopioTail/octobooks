...
import { Search, Package, Truck, CheckCircle, MapPin, Clock, Phone, AlertCircle } from 'lucide-react';
...
import { trackOrder, trackShipment, convertShiprocketTimeline, getShipmentStatus } from '@/lib/shipping';
  ...
  const handleTrackOrder = async () => {
    ...
    try {
      ...
      if (trackingInput.startsWith('ORD')) {
        // Track by order ID
        result = await trackOrder(trackingInput);
      } else if (trackingInput.startsWith('TRK')) {
        // Track by tracking number
        result = await trackShipment(trackingInput);
      } else {
        throw new Error('Please enter a valid Order ID (starting with ORD) or Tracking Number (starting with TRK)');
      }
      
      if (result.success) {
        setTrackingResult({
          orderId: trackingInput.startsWith('ORD') ? trackingInput : result.trackingData?.order_id || trackingInput,
          trackingNumber: result.awb || trackingInput,
          status: getShipmentStatus(result.status),
          estimatedDelivery: result.trackingData?.etd || '',
          currentLocation: result.trackingData?.current_location || '',
          timeline: convertShiprocketTimeline(result.timeline || [])
        });
      } else {
            ...
            <div className="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8">
              <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-6">
                <div className="flex items-center space-x-3">
                  ...
                  <div>
                    <h3 className="text-lg font-medium text-red-800 dark:text-red-200">Tracking Error</h3>
              ...
              <Card className="mb-8">
                ...
                <CardContent>
                  <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                    ...
                    <div className="text-center">
                      <Badge variant={getStatusBadgeVariant(trackingResult.status)} className="text-lg px-4 py-2">
                    ...
                    <div className="text-center">
                      <div className="text-lg font-semibold text-gray-900 dark:text-white">
                        {trackingResult.estimatedDelivery || 'Not available'}
                      </div>
